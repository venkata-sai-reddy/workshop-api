...
public class SkillsRepositoryImpl implements ISkillsRepo{
	...
	@Override
	public Boolean saveNewSkillsByAdmin(List<SkillVO> userSkills) throws GlobalException {
		int count = 0;
		int size = userSkills.size();
		MapSqlParameterSource[] batchArgs = new MapSqlParameterSource[size];
	    IntStream.range(0, size).forEach(i -> {
	        MapSqlParameterSource args = new MapSqlParameterSource();
	        args.addValue("skillName", userSkills.get(i).getSkillName());
	        args.addValue("status", Constants.APPROVED);
	        batchArgs[i] = args;
	    });
	    try {
			int[] updatedCount = namedParameterJdbcTemplate.batchUpdate(SqlProperties.skills.get("saveUserSkills"), batchArgs);
			log.debug("UserRepositoryImpl :: saveNewSkillsByAdmin(): {} Skills Updated", updatedCount);
			count = updatedCount.length;
	    } catch (DataAccessException exp) {
			log.error("UserRepositoryImpl :: saveNewSkillsByAdmin(): data access exception {}", exp.getMessage());
			throw new GlobalException(Constants.INTERNAL_SERVER_ERROR, HttpStatus.INTERNAL_SERVER_ERROR);
		} catch (Exception exp) {
			log.error("UserRepositoryImpl :: saveNewSkillsByAdmin(): exception {}", exp.getMessage());
			throw new GlobalException(Constants.INTERNAL_SERVER_ERROR, HttpStatus.INTERNAL_SERVER_ERROR);
		}
	    return count != 0; 
	}
