...
CREATE TABLE session_dtl (session_id serial PRIMARY KEY, user_id INT NOT NULL, session_time TIMESTAMP DEFAULT CURRENT_TIMESTAMP, session_end_time TIMESTAMP DEFAULT CURRENT_TIMESTAMP + interval ’15 minutes’, extend_count INT NOT NULL DEFAULT 0, FOREIGN KEY (user_id) REFERENCES user_prof (user_id) ON DELETE CASCADE);

CREATE TABLE venue_dtl (venue_id VARCHAR(10) PRIMARY KEY, name VARCHAR(50) NOT NULL, capacity NUMERIC (3));

CREATE TABLE workshop_dtl ( workshop_id serial PRIMARY KEY, name VARCHAR (100) NOT NULL, description VARCHAR (3000), location VARCHAR (100), capacity NUMERIC (3), enroll_count NUMERIC (3) DEFAULT 0, created_user_id INT NOT NULL, created_date TIMESTAMP DEFAULT CURRENT_TIMESTAMP, workshop_date DATE NOT NULL, start_time TIMESTAMP NOT NULL, end_time TIMESTAMP NOT NULL, FOREIGN KEY (created_user_id) REFERENCES user_prof (user_id) ON DELETE CASCADE, FOREIGN KEY (location) REFERENCES venue_dtl (venue_id) );
...
CREATE TABLE workshop_skill_info ( workshop_id INT , skill_id INT, PRIMARY KEY (workshop_id, skill_id), FOREIGN KEY (workshop_id) REFERENCES workshop_dtl (workshop_id) ON DELETE CASCADE, FOREIGN KEY (skill_id) REFERENCES skill_dtl (skill_id) );

CREATE TABLE workshop_register_dtl ( register_id serial PRIMARY KEY, workshop_id INT NOT NULL, reg_user_id INT NOT NULL, reg_date TIMESTAMP DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY (workshop_id) REFERENCES workshop_dtl (workshop_id) ON DELETE CASCADE, FOREIGN KEY (reg_user_id) REFERENCES user_prof (user_id) ON DELETE CASCADE);
...
INSERT INTO status_dtl (name) VALUES ('Notified');
INSERT INTO status_dtl (name) VALUES ('Completed');
INSERT INTO venue_dtl (venue_id, name, capacity) VALUES ('ONLINE', 'Online', '500');
INSERT INTO venue_dtl (venue_id, name, capacity) VALUES('JC101', 'Jonas Clark Hall 101', '30');
